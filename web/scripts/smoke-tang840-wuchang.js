...
let checks=0;
assert.equal(scenario.characters.length,434);assert.equal(Object.keys(archive.characters).length,434);checks+=2;
const ids=new Set();
...
assert.equal(scenario.characters.filter(c=>c.isHistorical).length,81);checks++;
console.log('PASS tang840-wuchang '+checks+' assertions; 434 people, 2170 configured values');
