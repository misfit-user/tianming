...
const raw=fs.readFileSync(path.join(work,'legacy-source.json')),before=JSON.parse(raw),out=clone(before);
const geometry=JSON.parse(fs.readFileSync(path.join(work,'r2-geometry.json'),'utf8'));
...
assert.equal(leaves.size,382);assert.equal(geometry.length,296);
...
newmap.authoringRevision='chongzhen-prefecture-r2';newmap.enabled=true;
newmap.id='tianqi-prefecture-refined-r2';
newmap.geographicReferences=JSON.parse(fs.readFileSync(path.join(work,'county-reference-report.json'),'utf8'));
newmap.refinementNotes={revision:'R2',unchangedCountryCoverage:true,redividedSamePolityProvinces:['ming-22','ming2-29','ming-27'],boundaryPrecision:'county-and-relief-constrained-inference',notSurveyed:true};
for(const c of registry)if(['ming-22','ming2-29','ming-27'].includes(c.sourceRegionId))c.note='在后金原控制范围内校正内部地理；府州并集重新形成省道，不改变对外边界。';
newmap.hierarchyPresentation.layerControl=true;
for(const ref of newmap.geographicReferences){
 if(!ref.accepted)continue;
 const target=rows.find(r=>r.accountingLeafIds.includes(ref.accountId));
 if(!target)throw Error('County alias missing account '+ref.accountId);
 const existing=aliases.filter(a=>a.text===ref.name).map(a=>a.regionId).concat(rows.filter(r=>r.name===ref.name).map(r=>r.id));
 if(existing.every(id=>id===target.id)){
  alias(ref.name,target.name);
  target.aliases=[...new Set((target.aliases||[]).concat(ref.name))];
 }
 target.aliases=[...new Set((target.aliases||[]).concat(ref.parentName+'·'+ref.name))];
 alias(ref.parentName+'·'+ref.name,target.name);
 alias(ref.parentName+ref.name,target.name);
}
for(const [text,name]of [['华亭','松江府'],['武陵','常德府'],['长山','济南府']])alias(text,name);
