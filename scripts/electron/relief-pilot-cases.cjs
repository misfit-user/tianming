...
module.exports=async function({win,temp,check,mode}){
  ...
  await js('(()=>{window.__reliefFullDigest=('+require('./full-graph-digest.cjs').toString()+');return true;})()');
  ...
  await check('relief-closing-draft-and-viewing-register-preserves-draft-and-world',async()=>{
    const r=await js(`(async()=>{TMPhase8FormalBridge.drafts.closeDeskOverlay();function graph(){return __reliefFullDigest({GM,P});}__reliefTest.graph=graph;const before=await graph();console.log('RELIEF_GRAPH_DIGEST_DONE',before.length);openShizhengTasks();console.log('RELIEF_OPENED');const unchanged=before===await graph();console.log('RELIEF_COMPARE_DONE');closeShizhengTasks();openZhao();return{unchanged,draft:document.querySelector('#tm-action-edict-overlay #edict-eco').value};})()`);
  ...
  await check('relief-original-turn-collection-not-viewing-register-creates-the-edict',async()=>{
    const r=await js(`(async()=>{const input=_endTurn_collectInput();const e=GM._edictTracker.find(x=>x.content===__reliefTest.text);if(!e)throw Error('missing-original-edict');__reliefTest.edictId=e.id;TMPhase8FormalBridge.drafts.closeDeskOverlay();const before=await __reliefTest.graph();openShizhengTasks();const result=TM.ReliefGovernance.list(GM,{reliefOnly:false});return{input:input.edicts.economic,exists:result.entries.some(x=>x.sourceId===e.id),unchanged:before===await __reliefTest.graph()};})()`);
  ...
  await check('populated-relief-sources-do-not-recreate-the-removed-UI',async()=>{
    const r=await js(`(async()=>{const before=await __reliefTest.graph();openShizhengTasks();const p=document.getElementById('shizheng-tasks-overlay');return {same:before===await __reliefTest.graph(),retired:document.querySelectorAll('[data-relief-register],[data-relief-channel]').length,text:p.textContent,source:GM._edictTracker.find(e=>e.id===__reliefTest.edictId)?.content};})()`);
