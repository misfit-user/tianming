...
- 还账工具 `codemod-design-tokens.js`（第1刀）：只把与色板令牌**逐位相等**的颜色换成令牌，半透明写成
  `rgba(var(--gold-400-rgb),.5)`，画面不变；跳过 `url()`、注释、色板自己的定义。只能跑纯样式文本
  （`.css`、`phase8-formal-bridge-styles.js` 这类整份是样式字符串的注入族），画布、SVG 属性不认 `var()`。
