...
app.whenReady().then(async () => {
  ...
  try {
    ...
    report.liveOwnership = await js("(()=>{const api=TMPhase8FormalBridge.map,map=api.getMapData(),r=map.regions[0],gm=window.GM,previous=gm._provinceToFaction;const table=previous||{},key=r.id||r.name,had=Object.prototype.hasOwnProperty.call(table,key),value=table[key];gm._provinceToFaction=table;try{const old=api.findFaction(api.ownerKey(r),r.factionName||r.ownerName),oldKey=old&&(old.stableOwnerKey||old.mapFactionId||old.id),target=Object.keys(map.factions||map.factionColors||{}).find(k=>k!==oldKey&&api.findFaction(k));if(!target)throw Error('No distinct registered faction for probe');const before=api.__formalMapSignature(map);table[key]=target;const owner=api.ownerKey(r),changed=api.__formalMapSignature(map)!==before;return {owner,target,oldKey,signatureChanged:changed};}finally{if(had)table[key]=value;else delete table[key];gm._provinceToFaction=previous;}})()");
    if (report.liveOwnership.owner !== report.liveOwnership.target || !report.liveOwnership.signatureChanged) throw Error('Live ownership visibility regressed');
