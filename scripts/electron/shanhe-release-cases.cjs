...
module.exports=async function({win,check}){
 ...
 // 三层地图在后台准备，准备期间地图不接滚轮；要等三层真正上屏再测真实滚轮
 await js(`(async()=>{const end=Date.now()+60000;while(!(TMPhase8FormalBridge.map.preparationStatus()?.ready&&document.querySelector('#ming-map-layer > .tmf-prepared-map-surface #tmf-formal-map'))){if(Date.now()>end)throw Error('prepared map layers did not come up');await new Promise(r=>setTimeout(r,100));}})()`);
