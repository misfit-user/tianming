...
try {
  ...
  if (argv.includes('--circuit-book')) modes.splice(0, modes.length, 'circuit-book');
  if (argv.includes('--circuit-tour')) modes.splice(0, modes.length, 'circuit-tour');
  ...
  for (const mode of modes) {
    ...
    const run = cp.spawnSync(runtime, [...electronArgs, path.join(__dirname, 'electron/bridge-main.cjs')], { cwd: repo, env, encoding: 'utf8', windowsHide: true, timeout: ['authoring-efficiency','relief-pilot'].includes(mode) ? 620000 : ['authoring-boundaries','authoring-continuation','authoring-autoapply','authoring-efficiency'].includes(mode) ? 195000 : mode === 'strategic-map' ? 620000 : mode === 'shanhe-load' ? 255000 : mode === 'circuit-book' || mode === 'circuit-tour' ? 315000 : mode === 'map-core-controls' || mode === 'startup-project-fallback' || mode === 'authoring-regions' ? 195000 : mode === 'shanhe-map' || mode === 'shanhe-pointer' ? 150000 : mode === 'relief-inspect' ? 1860000 : mode === 'relief-pilot' ? 210000 : mode === 'native-start-neutral-atlas' ? 210000 : 90000, maxBuffer: 8 * 1024 * 1024 });
