...
launches: for (let i = 0; i < repeats; i++) for (const side of (i % 2 ? ['after', 'before'] : ['before', 'after'])) {
  ...
  const env = { ...process.env, TM_BRIDGE_TEST_ROOT: repo, TM_BRIDGE_TEST_MODE: 'performance', TM_BRIDGE_TEST_REPORT: file,
    TM_BRIDGE_TEST_USERDATA: userData, TM_PERF_SAMPLE_DIR: path.join(temporary, 'shared-samples'), TM_PERF_ITERATION: String(i) };
  ...
  const record = { side, iteration: i, head: cp.execFileSync('git', ['rev-parse', 'HEAD'], { cwd: repo, encoding: 'utf8' }).trim(),
    sourceHashes: Object.fromEntries(['web/tm-storage.js', 'web/phase8-formal-map.js', 'web/tm-save-lifecycle.js'].map(p => [p, crypto.createHash('sha256').update(fs.readFileSync(path.join(repo, p))).digest('hex')])),
