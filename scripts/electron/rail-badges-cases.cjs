...
module.exports=async function({win,check}){
  ...
  await check('same-person-queue-and-dynamic-request-is-not-double-counted',async()=>{await js(`(()=>{GM._pendingAudiences.push({name:'请见文臣甲',_qid:'a1'});TMPhase8FormalBridge.openRailPanel('issue');})()`);assert.equal((await badge()).text,'3');assert.equal(await js(`document.querySelectorAll('#tm-phase8-formal-panel [data-right-action="wendui-audience"]').length`),0);await js(`(()=>{_wdRemovePendingAudience('a1');TMPhase8FormalBridge.openPanel('issue');})()`);});
