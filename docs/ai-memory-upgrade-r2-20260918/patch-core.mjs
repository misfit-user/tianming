...
for (const file of ['web/index.html','web/editor.html']) edit(file, (s, r) => {
  const match = s.match(/<script[^>]+src="tm-ai-infra-model-detect\.js[^\"]*"[^>]*><\/script>/);
  if (!match) throw Error('model detection loader missing: ' + file);
  return r(s, match[0], ['tm-memory-adaptive.js','tm-memory-long-term.js','tm-memory-hybrid.js'].map(name => '<script src="' + name + '?v=20260918-memory-r2"></script>').join('\n') + '\n' + match[0]);
});
