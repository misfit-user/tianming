...
module.exports = async function ({win, root, check}) {
  ...
  await check(sid+' terrain loads while optional label geometry is still pending', async()=>{
    const d=await active();assert(d.active,JSON.stringify(d));
    assert.equal(await js('typeof TMMapRealmLayout'),'undefined');
    assert.equal(d.regions,entry.counts.regions);assert.match(d.button,/2\.5D/);
  });
