 ...
 *
 * Default: verified copies/excerpts of 3f8065cb...; only temp files and the JSON report are written.
...
function loadMain(names, dependencies = {}) {
  const context = vm.createContext(Object.assign({ fs, path, crypto, process, nodeNet, console: quiet }, dependencies));
...
function runCi(report) {
  ...
  const context = vm.createContext({
    __dirname: '/audit-fixture/web/scripts',
    require(name) {
      if (name === 'child_process') return { execSync() { throw new Error('Injected smoke runner failure'); } };
      if (name === 'path') return path;
      if (name === 'fs') return { readFileSync(file) {
        return JSON.stringify(String(file).includes('ci-smoke-allowlist') ? ['smoke-audio-bgm.js'] : report);
      }};
      throw new Error('Unexpected CI dependency: ' + name);
    },
    console: { log: (...v) => messages.push(v.join(' ')), error: (...v) => messages.push(v.join(' ')) },
    process: { exit(code) { exitCode = code; const e = new Error('exit'); e.testExit = true; throw e; } }
  });
  try { new vm.Script(ciSource).runInContext(context, { timeout: 2000 }); } catch (e) { if (!e.testExit) throw e; }
...
(async () => {
  ...
  await probe('TM-AUD-02', 'workshop overwrite loses old package when copying fails', async () => {
    const root = tempCase(), packs = path.join(root, 'packs'), source = path.join(root, 'incoming'), target = path.join(packs, 'audit-pack');
  ...
  await probe('TM-AUD-08', 'export write failure destroys an existing backup payload', async () => {
    ...
    const faultyFs = Object.assign({}, fs, { writeFileSync(file, ...rest) {
      if (file === dest) { fs.writeFileSync(dest, '{'); throw Object.assign(new Error('Injected partial write ENOSPC'), { code: 'ENOSPC' }); }
      return fs.writeFileSync(file, ...rest);
    }});
    const handlers = {};
    const c = vm.createContext({ fs: faultyFs, mainWindow: {},
  ...
  fs.writeFileSync(path.join(__dirname, 'repro-results.json'), JSON.stringify(output, null, 2) + '\n');
