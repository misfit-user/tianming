...
// authoring-regions 实测 57～80 秒（CI 与本机，09-24），默认 75 秒会随机超时，与其他重型用例一样单列 180 秒
setTimeout(() => finish(new Error('electron-bridge-timeout')), (multiScenarioAuthoring && mode === 'authoring-efficiency') || mode === 'relief-pilot' ? 600000 : multiScenarioAuthoring ? 180000 : mode === 'strategic-map' ? 600000 : mode === 'shanhe-load' ? 240000 : mode === 'circuit-book' || mode === 'circuit-tour' ? 300000 : mode === 'map-core-controls' || mode === 'startup-project-fallback' || mode === 'authoring-regions' ? 180000 : mode === 'shanhe-map' || mode === 'shanhe-pointer' ? 135000 : mode === 'native-start-live-authoring' ? 570000 : mode === 'performance-inspect' || mode === 'relief-inspect' ? 1800000 : mode === 'relief-pilot' || mode === 'native-start-neutral-atlas' ? 180000 : visiblePerformance ? 240000 : 75000);
...
app.on('browser-window-created', (_event, win) => {
  ...
  win.webContents.once('did-finish-load', async () => {
    try {
      ...
      else if (mode === 'circuit-book') await require('./circuit-book-cases.cjs')({ win, check });
      else if (mode === 'circuit-tour') await require('./circuit-tour-cases.cjs')({ win, check });
