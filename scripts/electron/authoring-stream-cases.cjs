...
module.exports = async function({ win, root, check }) {
  ...
  await until(`TM_AuthoringAgentUI._ui.els.summary.textContent.includes('已完成国库设定修改')`);
  ...
  await js(`(()=>{
    ...
    window.fetch=async function(url,options){
      ...
      const selected=p.round===1?tool('applyEdit',{path:'fiscalConfig.treasury',value:4500000}):tool('finish',{summary:'重试已完整修改国库。'});
      const event=d=>'data: '+JSON.stringify(d)+'\\n\\n';
  ...
  await check('authoring-stream-real-stop-cancels-body-and-does-not-apply-prefix', async () => {
    assert.equal(await js(`__aaStream.cancelled===1 && __aaStream.round===1 && JSON.stringify(TM_SCENARIO_EDITOR_RESET_APP.state.scenario)===__aaStream.before`), true);
  });
};
