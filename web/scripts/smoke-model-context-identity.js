...
test('switching an unknown relay model cannot inherit old 4K detection',()=>assert.equal(read({_detectedContextK:4,_ctxCacheKey:'relay-old@https://relay.invalid/v1'}),256));
test('same model on another endpoint cannot inherit old capacity',()=>assert.equal(read({_detectedContextK:1024,_ctxCacheKey:'relay-new@https://other.invalid/v1'}),256));
test('legacy unowned detection is not treated as current model evidence',()=>assert.equal(read({_detectedContextK:4}),256));
...
test('invalid nonfinite and negative values cannot poison prompt/compression budgets',()=>{for(const value of [Infinity,'Infinity',NaN,-4,'bad'])assert.equal(read({contextSizeK:value,_detectedContextK:value,_ctxCacheKey:'relay-new@https://relay.invalid/v1'}),256);});
test('secondary detections never become primary prompt capacity',()=>assert.equal(read({_detectedContextK_secondary:4,_ctxCacheKey_secondary:'relay-new@https://relay.invalid/v1'}),256));
