...
app.whenReady().then(async()=>{win=new BrowserWindow({width:2400,height:1500,show:false,webPreferences:{contextIsolation:true,nodeIntegration:false,sandbox:true,backgroundThrottling:false}});
 ...
 check('official-loader-current-candidate',load.regions===296&&load.provinces===43&&load.characters===203&&load.armies===48,load);mark('official-loaded',load);
 ...
 const world=await js(`({running:GM.running,regions:GM.mapData?.regions?.length,player:GM.playerInfo?.characterName,characters:GM.chars?.length,armies:GM.armies?.length})`);check('native-world-running',world.running&&world.regions===296,world);report.world=world;
 async function waitMap(){const start=Date.now();let info;do{info=await js(`(()=>{const svg=document.getElementById('tmf-formal-map');return {paths:svg?.querySelectorAll('.tmf-region').length||0,scale:document.getElementById('mapwrap')?.dataset.mapScale,message:document.getElementById('ming-map-layer')?.innerText?.slice(0,120)};})()`);if(info.paths===296)return info;if(Date.now()-start>90000)throw Error('map preparation timed out '+JSON.stringify(info));await sleep(900);}while(!done);}
 ...
 for(const tier of ['realm','region','prefecture']){
  await js(`document.querySelector('.map-scale[data-map-scale="${tier}"]').click();document.querySelector('[data-map-fit-all]').click()`);await sleep(1600);const info=await waitMap();check('tier-'+tier,info.scale===tier&&info.paths===296,info);
  ...
  check('panorama-'+tier,viewport.paths===296&&viewport.clipped.length===0,viewport);
  ...
  await win.webContents.capturePage();await sleep(750);
  await js(`(()=>{document.getElementById('chongzhen-map-proof').textContent='崇祯 R2 · '+({realm:'天下',region:'省道',prefecture:'府州'})['${tier}']+'全景｜本地原生游戏｜固定取景｜隔离数据';return new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve(true))));})()`);
 ...
 await js(`TMPhase8FormalBridge.map.focusRegion('ming-22-p01')`);await sleep(1800);await win.webContents.capturePage();await sleep(750);
 fs.writeFileSync(path.join(dir,'map-liaodong-detail.png'),(await win.webContents.capturePage()).toPNG());
 report.liveTransfers=await js(fs.readFileSync(path.join(root,'docs/chongzhen-map-r2-20260918/verify-live-transfers.js'),'utf8'));
