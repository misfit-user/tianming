...
func _ready() -> void:
	...
	if panel == null:
		...
		return
	if not panel.has_signal("court_recommendation_requested"):
		_fail("Court meeting panel does not expose recommendation requests")
		return
	...
	var pending: Array = _array(game_state.get("pending_court_recommendations"))
	if pending.is_empty():
		_fail("Court meeting UI did not create pending recommendations")
		return
	var recommendation_id: String = str(_dict(pending[0]).get("id", ""))
	var action_points_before: int = int(game_state.get("action_points"))
	panel.emit_signal("court_recommendation_requested", recommendation_id)
	await get_tree().process_frame

	if int(game_state.get("action_points")) != action_points_before - 1:
		_fail("Recommendation UI request did not spend one action point")
		return
	if _array(game_state.get("enacted_court_recommendations")).is_empty():
		_fail("Recommendation UI request did not record enacted recommendation")
		return

	print("[TianmingGodotTest] court meeting recommendation UI scene test passed")
...
func _fail(message: String) -> void:
	print("[TianmingGodotTest] court meeting recommendation UI scene test failed: %s" % message)
	push_error(message)
	get_tree().create_timer(5.0).timeout.connect(func() -> void: _finish(1))

func _finish(exit_code: int) -> void:
	print("[TianmingGodotTest] court meeting recommendation UI scene test exit_code=%d" % exit_code)
