...
(function(root) {
  ...
  function lexical(hits, query, limit) {
    ...
    var docs = hits.map(function(hit) {
      var words = tokens(body(hit) + ' ' + (hit.entities || []).join(' ') + ' ' + (hit.char || '')), counts = Object.create(null);
      words.forEach(function(word) { counts[word] = (counts[word] || 0) + 1; });
      q.forEach(function(word) { if (counts[word]) df[word] = (df[word] || 0) + 1; });
      totalLength += words.length; return { hit: hit, counts: counts, length: words.length };
    });
  ...
  function fuse(lexicalRows, vectors, topK) {
    ...
    var candidates = Object.keys(pool).map(function(id) { return pool[id]; }), selected = [];
    while (selected.length < topK && candidates.length) {
      candidates.forEach(function(row) { var similarity = selected.reduce(function(max, s) { return Math.max(max, overlap(row.hit, s.hit)); }, 0); row.utility = row.score * (1 - 0.35 * similarity); });
