...
app.whenReady().then(async()=>{win=new BrowserWindow({width:2400,height:1500,show:false,webPreferences:{contextIsolation:true,nodeIntegration:false,sandbox:true,backgroundThrottling:false}});
 ...
 check('official-loader-current-candidate',load.regions===307&&load.provinces===43&&load.characters===203&&load.armies===48,load);mark('official-loaded',load);
 ...
 const world=await js(`({running:GM.running,regions:GM.mapData?.regions?.length,player:GM.playerInfo?.characterName,characters:GM.chars?.length,armies:GM.armies?.length})`);check('native-world-running',world.running&&world.regions===307,world);report.world=world;
 async function waitMap(){const start=Date.now();let info;do{info=await js(`(()=>{const svg=document.getElementById('tmf-formal-map');return {paths:svg?.querySelectorAll('.tmf-region').length||0,scale:document.getElementById('mapwrap')?.dataset.mapScale,message:document.getElementById('ming-map-layer')?.innerText?.slice(0,120)};})()`);if(info.paths===307)return info;if(Date.now()-start>90000)throw Error('map preparation timed out '+JSON.stringify(info));await sleep(900);}while(!done);}
 ...
 await js(`(()=>{const state=TMPhase8FormalBridge.__p8MapParts.state;if(!state)throw Error('Native camera state unavailable');const fit=document.querySelector('[data-map-fit-all]');if(!fit||getComputedStyle(fit).display==='none')throw Error('Visible full-map button missing');fit.click();const dock=document.getElementById('map-tools-dock');if(dock?.classList.contains('open'))document.querySelector('[data-map-tools-toggle]').click();})()`);
 const controls=await js(`(()=>{const s=TMPhase8FormalBridge.__p8MapParts.state,b=document.querySelector('[data-map-tier-lock]');return {locked:s._zoomLevelLinkOff,text:b.textContent,shown:getComputedStyle(b).display!=='none',scale:s.mapView.scale};})()`);
 ...
 for(const tier of ['realm','region','prefecture']){
  await js(`document.querySelector('.map-scale[data-map-scale="${tier}"]').click();document.querySelector('[data-map-fit-all]').click()`);await sleep(1600);const info=await waitMap();check('tier-'+tier,info.scale===tier&&info.paths===307,info);
  ...
  check('panorama-'+tier,viewport.paths===307&&viewport.clipped.length===0,viewport);
  ...
  await win.webContents.capturePage();await sleep(750);
  await js(`(()=>{document.getElementById('chongzhen-map-proof').textContent='崇祯 R3 · '+({realm:'天下',region:'省道',prefecture:'府州'})['${tier}']+'全景｜本地原生游戏｜固定取景｜隔离数据';return new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve(true))));})()`);
 ...
 report.jinzhouBundle=await js(`(()=>{const r=GM.mapData.regions.find(r=>r.id==='ming-28-p03');const b=TMPhase8FormalBridge.__p8MapParts.regionBundle(r);return {adminId:b.liveDivision?.id,pop:b.pop,gender:b.data.byGender};})()`);
 check('live-dossier-does-not-read-parent-population',report.jinzhouBundle.adminId==='div_pref_ming_28_03'&&report.jinzhouBundle.pop.ding===12723&&report.jinzhouBundle.pop.mouths===48936,report.jinzhouBundle);
 await js(`TMPhase8FormalBridge.map.focusRegion('ming-28-p03');document.getElementById('chongzhen-map-proof').textContent='崇祯 R3 · 关宁防线方志｜本地原生游戏｜隔离数据';for(let i=0;i<4;i++)document.querySelector('[data-map-zoom="1.22"]').click();TMPhase8FormalBridge.map.focusRegion('ming-28-p03')`);await sleep(1800);await win.webContents.capturePage();await sleep(750);
 fs.writeFileSync(path.join(dir,'map-liaodong-detail.png'),(await win.webContents.capturePage()).toPNG());
 await js(`TMPhase8FormalBridge.map.focusRegion('ming-28-p08'); TMPhase8FormalBridge.map.openRegionDossier(GM.mapData.regions.find(r=>r.id==='ming-28-p08'));document.getElementById('chongzhen-map-proof').textContent='崇祯 R3 · 皮岛与东江方志｜本地原生游戏｜隔离数据'`);
 await sleep(1800);await win.webContents.capturePage();await sleep(750);
 report.pidaoAccounts=await js(`(()=>{const r=GM.mapData.regions.find(r=>r.id==='ming-28-p08');const b=TMPhase8FormalBridge.__p8MapParts.regionBundle(r);return {population:b.pop.mouths,ding:b.pop.ding,count:b.data._sourceAccountCount,note:b.data.description};})()`);
 check('pidao-shared-account-population-not-first-child',report.pidaoAccounts.population===170214&&report.pidaoAccounts.ding===44256&&report.pidaoAccounts.count===4,report.pidaoAccounts);
 const pidaoPanel=await js(`(()=>{const p=document.getElementById('ppop');return {regionId:p?.dataset.regionId,text:p?.innerText?.slice(0,200)};})()`);
 check('pidao-dossier-real-name-and-id',pidaoPanel.regionId==='ming-28-p08'&&pidaoPanel.text.includes('皮岛')&&!pidaoPanel.text.includes('未名地块'),pidaoPanel);
 await win.webContents.capturePage();await sleep(750);
 ...
 report.liveTransfers=await js(fs.readFileSync(path.join(root,'docs/chongzhen-map-r3-20260918/verify-live-transfers.js'),'utf8'));
