...
/**
 * editor-division-deep.js — 行政区划编辑器深化字段扩展
 *
...
(function(global) {
  ...
  function _renderDisasterRow(rec, idx) {
    rec = rec || {};
    var typeOpts = DISASTER_TYPES.map(function(t) {
      return '<option value="' + t.val + '"' + (rec.type === t.val ? ' selected' : '') + '>' + t.label + ' (' + t.val + ')</option>';
    }).join('');
    ...
    var h = '<div class="dd-disaster-row" data-dis-idx="' + idx + '" style="display:grid;grid-template-columns:90px 60px 80px 1fr 28px;gap:4px;align-items:center;padding:3px;background:#1d1d1d;border-left:2px solid #d4a838;border-radius:3px;">';
    h += '<select class="dd-dis-type" style="padding:3px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;font-size:11px;">' + typeOpts + '</select>';
    h += '<select class="dd-dis-sev" style="padding:3px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;font-size:11px;">' + sevOpts + '</select>';
    h += '<input class="dd-dis-turn" type="number" min="1" placeholder="回合" value="' + (rec.startTurn || '') + '" style="padding:3px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;font-size:11px;">';
    h += '<input class="dd-dis-note" type="text" placeholder="备注" value="' + _esc(rec.note || '') + '" style="padding:3px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;font-size:11px;">';
    h += '<button type="button" onclick="event.preventDefault();this.parentElement.remove()" style="background:transparent;color:#c04030;border:1px solid #c04030;border-radius:3px;cursor:pointer;font-size:11px;padding:1px 4px;">×</button>';
    h += '</div>';
    return h;
  }
  ...
  function _ensureDivisionDeepFields(node) {
    ...
    // schema v2·与 map-editor-core.js createDivision() 对齐
    if (!node.tags) {
      node.tags = {
        hasPort: false, saltRegion: false, mineralRegion: false,
        horseRegion: false, fishingRegion: false, imperialDomain: false
      };
    }
    if (!node.economyBase) {
      node.economyBase = {
        farmland: 0, commerceCoefficient: 1.0, commerceVolume: 0,
        maritimeTradeVolume: 0, saltProduction: 0, mineralProduction: 0,
        horseProduction: 0, fishingProduction: 0,
        imperialFarmland: 0, imperialAssets: { zhizao: 0, kuangchang: 0, yuyao: 0 },
        postRelays: 0, kejuQuota: 0, roadQuality: 50,
        landsAnnexed: 0, landsReclaimed: 0, landsSurveyed: 0,
        disasterRecord: []
      };
    }
  }
  ...
  function renderDivisionDeepFieldsHTML(node) {
    ...
    h += '<div><label style="font-size:11px;">道路质量(0-100)</label><input type="number" id="dd-eb-roadQuality" min="0" max="100" value="' + (eb.roadQuality != null ? eb.roadQuality : 50) + '" style="width:100%;padding:4px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;" title="商旅/军调/驿递成本系数"></div>';
    h += '</div>';
    // 田亩流转·诚实账（清丈/兼并/开垦诏令的 ledger）
    h += '<div style="margin-top:10px;color:#bbb;font-size:12px;">田亩流转（兼并·开垦·清丈累计）</div>';
    h += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;">';
    h += '<div><label style="font-size:11px;">兼并田(亩)</label><input type="number" id="dd-eb-landsAnnexed" min="0" value="' + _fmtNum(eb.landsAnnexed) + '" style="width:100%;padding:4px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;" title="豪强勋戚兼并·脱税基"></div>';
    h += '<div><label style="font-size:11px;">开垦田(亩)</label><input type="number" id="dd-eb-landsReclaimed" min="0" value="' + _fmtNum(eb.landsReclaimed) + '" style="width:100%;padding:4px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;" title="劝农开垦·入税基"></div>';
    h += '<div><label style="font-size:11px;">清丈田(亩)</label><input type="number" id="dd-eb-landsSurveyed" min="0" value="' + _fmtNum(eb.landsSurveyed) + '" style="width:100%;padding:4px;background:#1a1a1a;border:1px solid #444;color:#e0e0e0;" title="清丈诏令成果·恢复税基"></div>';
    h += '</div>';
    // 在灾实录·dynamic rows·6 类灾种 × 严重度 × 起始回合 + 备注
    h += '<div style="margin-top:12px;display:flex;align-items:center;justify-content:space-between;">';
    h += '<span style="color:#bbb;font-size:12px;">在灾实录 (disasterRecord)</span>';
    h += '<button type="button" onclick="event.preventDefault();window._dd_addDisasterRow()" style="padding:2px 10px;background:#3a3024;color:#d4a838;border:1px solid #d4a838;border-radius:3px;font-size:11px;cursor:pointer;">+ 添灾</button>';
    h += '</div>';
    h += '<div id="dd-disaster-rows" style="margin-top:6px;display:flex;flex-direction:column;gap:4px;">';
    var dr = (eb.disasterRecord && Array.isArray(eb.disasterRecord)) ? eb.disasterRecord : [];
    dr.forEach(function(rec, idx){ h += _renderDisasterRow(rec, idx); });
    h += '</div>';
    h += '</details>';
  ...
  function collectDivisionDeepFromForm(node) {
    ...
    var ebRq = num('dd-eb-roadQuality'); if (ebRq !== null) node.economyBase.roadQuality = ebRq;
    var ebLa = num('dd-eb-landsAnnexed'); if (ebLa !== null) node.economyBase.landsAnnexed = ebLa;
    var ebLr = num('dd-eb-landsReclaimed'); if (ebLr !== null) node.economyBase.landsReclaimed = ebLr;
    var ebLs = num('dd-eb-landsSurveyed'); if (ebLs !== null) node.economyBase.landsSurveyed = ebLs;
    // 在灾·从 DOM 行重建数组
    var rowsCtn = document.getElementById('dd-disaster-rows');
    if (rowsCtn){
      var rows = rowsCtn.querySelectorAll('.dd-disaster-row');
      var arr = [];
      rows.forEach(function(row){
        var typ = (row.querySelector('.dd-dis-type') || {}).value || '';
        var sev = Number((row.querySelector('.dd-dis-sev') || {}).value || 1);
        var turn = Number((row.querySelector('.dd-dis-turn') || {}).value || 0);
        var note = (row.querySelector('.dd-dis-note') || {}).value || '';
        if (!typ) return;
      ...
      node.economyBase.disasterRecord = arr;
    }
  }
  ...
  async function aiGenDivisionDeep(btn) {
    ...
    try {
      ...
      var prompt = '你是中国历史行政区划专家。按' + dynasty + '(' + era + ')时期的真实数据，为区划"' + _esc(name) + '"（级别:' + level + '，地形:' + terrain + '，总人口约' + pop + '）生成深化字段。\n\n要求严格返回 JSON：\n' +
        ...
        '  "economyBase": {"farmland":亩数,"commerceVolume":商业绝对量,"commerceCoefficient":0.3边远~5.0京师,"maritimeTradeVolume":海贸量(无港=0),"saltProduction":年产盐斤(非产盐=0),"mineralProduction":两/年(非产矿=0),"horseProduction":年产马匹(非草场=0),"fishingProduction":两/年(非渔区=0),"imperialFarmland":皇庄亩数(非皇室直辖=0),"imperialAssets":{"zhizao":织造局数,"kuangchang":矿场数,"yuyao":御窑数},"postRelays":驿站数,"kejuQuota":解额数,"roadQuality":0-100道路质量,"landsAnnexed":兼并田亩数,"landsReclaimed":开垦田亩数,"landsSurveyed":清丈田亩数}\n' +
      ...
      if (data.economyBase) {
        ...
        setVal('dd-eb-roadQuality', data.economyBase.roadQuality);
        setVal('dd-eb-landsAnnexed', data.economyBase.landsAnnexed);
        setVal('dd-eb-landsReclaimed', data.economyBase.landsReclaimed);
        setVal('dd-eb-landsSurveyed', data.economyBase.landsSurveyed);
      }
  ...
  // 给 inline onclick 用·添新灾行
  global._dd_addDisasterRow = function(){
    var ctn = document.getElementById('dd-disaster-rows');
    if (!ctn) return;
    var nextIdx = ctn.querySelectorAll('.dd-disaster-row').length;
    var wrap = document.createElement('div');
    wrap.innerHTML = _renderDisasterRow({}, nextIdx);
    ctn.appendChild(wrap.firstChild);
  };
