...
// 用法（仓库根目录）：node docs/scenario-data-repair-20260924/tools/shaosong-brief.js <路名> [原版剧本 json]
...
const leaves = base.adminHierarchy.player.divisions[0].children.filter((l) => regions.get(l.mapRegionId).circuitName === circuit);
...
leaves.forEach((l) => {
  ...
  const hh = sources.householdWeight(l.name);
  const comm = sources.commerceWeight(l.name, circuit);
