...
if (selectedCase === null) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (child.status !== 0 || !result) { failed++; console.error('CASE_PROCESS_FAILURE', JSON.stringify({ index: i, status: child.status, signal: child.signal, error: child.error && child.error.message })); } if (result) { passed += result.passed; failed += result.failed; }
  }
...
for (const s of (selectedCase < 4 ? [scenario(Math.floor(selectedCase / 2))] : [])) for (const long of [selectedCase % 2 === 1]) {
  ...
  check(s.name + ' long=' + long + ': actual normalizers/serializers idb + project equal legacy preparation', () => {
    ...
    for (const format of ['idb', 'project']) {
      ...
      if (format === 'idb') { const bytes = Buffer.from(actualText, 'utf8'); savedIdb = { actual: bytes, expected: bytes }; idbCloneCounts = { legacy: legacyClones, current: actualClones }; }
      expectedText = null;
  ...
  check(s.name + ' long=' + long + ': actual restore preserves canonical and legacy mirrors', () => {
    function restore(value) {
      const r = saveBuilder(root); const roundtrip = JSON.parse(Buffer.isBuffer(value) ? value.toString('utf8') : typeof value === 'string' ? value : JSON.stringify(value)); r.c.GM = roundtrip.GM; r.c.P = roundtrip.P;
