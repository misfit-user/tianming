...
func _ready() -> void:
	...
	var setup: Dictionary = _prepare_appointment_relation(game_state)
	if setup.is_empty():
	...
	if not before_text.contains(str(setup.get("target_name", ""))) or not before_text.contains("10"):
		_fail("Relationship panel did not show the seeded appointment relation")
		return

	var result: Dictionary = game_state.call("appoint_character", str(setup.get("target_id", "")), str(setup.get("office_id", "")))
	...
	if not after_text.contains(str(setup.get("target_name", ""))) or not after_text.contains("12"):
		_fail("Relationship panel did not refresh the synced appointment relation")
		return
	if not after_text.contains("任命"):
		_fail("Relationship panel did not show the appointment sync description")
		return

	print("[TianmingGodotTest] relationship panel appointment refresh scene test passed")
	_finish(0)
...
func _prepare_appointment_relation(state: RefCounted) -> Dictionary:
	...
	state.set("character_relations", [{
		"id": "test_relationship_panel_appointment",
		"from": emperor_name,
		"to": target_name,
		"type": "君臣",
		"value": 10,
...
func _fail(message: String) -> void:
	print("[TianmingGodotTest] relationship panel appointment refresh scene test failed: %s" % message)
	push_error(message)
	_finish(1)

func _finish(exit_code: int) -> void:
	print("[TianmingGodotTest] relationship panel appointment refresh scene test exit_code=%d" % exit_code)
