...
(function (root) {
  ...
  function recordSpecs(ctx) {
    ...
    return {
      ...
      shilu: "实录" + sh[0] + "-" + sh[1] + "字——正史实录体·纯文言(仿历代实录与《资治通鉴》)。以干支系日、'朔'纪月首(如'某月庚辰朔…壬午…甲申')依时序排比；纪事以君为纲:君称'上'，叙其视朝('上御殿')、诏谕('上诏''上谕曰''敕曰')、裁断('从之''不从''留中''下有司议')；臣下称官衔加名，奏对作'某官疏言……''得旨……'；动词用诏/谕/敕/命/召/赐/擢/罢/黜/调/夺/赠/恤等实录套语，数目用汉字(如一百六十万)，以'是月''是日''先是''初'缀连成纪。党派进退、弹劾成败、民请汹汹亦国之大事，宜入纪(如'某党柄政''以劾罢某官''某地民伏阙请减赋')。铁律:只记可验证之事(诏令/任免/战事/灾异/人事大变)，记事不评论、不抒情、不议得失(评断归政文)，禁白话与现代语、禁口语虚词。",
      szjTitle: "时政记副标题——七字对仗两句，概括本回合主题(如'雷霆除藩安豫地，断禄激变祸萧墙'；两句用'，'分隔)",
      shizhengji: "时政记正文" + sz[0] + "-" + sz[1] + "字——仿崇祯朝政纪要体：\\n  1.开篇总括：'陛下本回合……颁布数道谕旨：其一……；其二……'，逐条复述玩家诏令/私人行动\\n  2.按领域分段(3-5段)——军事与边防/内政与民生/吏治与人事/宗室与外戚/关外局势等，每段开头用【军事】【朝政】【经济】【外交】【民生】【宫廷】【党争】等方括号标签\\n  3.每段必须完整因果链：诏令→执行者→执行过程→阻力/意外→实际效果→遗留隐患。不要只写结果，要写过程和阻碍\\n  4.跨回合延续：用'此前''原本''延续'衔接往期决策的后续影响\\n  5.自然融入信息源：据XX奏报/有司呈报/密探来报/坊间传言/边军塘报\\n  6.党争与民情是与诏令并行的主线：政柄更迭(某党秉政/某党见逐于朝)、弹劾成败、党派清议须入【党争】或【朝政】段；某阶层怨望日深、民间请愿运动成势/鼎沸须入【民生】段——依系统状态帐(党派数值/阶层正册/运动)如实叙，数值与叙事同帐，勿凭空造党争\\n  段间用\\n\\n分隔。",
  ...
  function hourenSpec(ctx) {
    ...
    return "\n基于上述全部资料，撰写《后人戏说》——这是玩家角色本回合的完整生活进程，核心目的是**完整、立体地呈现玩家角色的日常生活**，让玩家看见自己的角色如何度过这一段时光。\n"
      ...
      + "  · 党争朝局：若本回合有党派倾轧/弹劾/政柄更迭(见事实清单)，化为具体人物的得意与惶惶(谁夜访谁、谁在值房摔了茶盏、谁称病不朝)——不写术语，写人\n"
      + "  · 阶层民情与民间运动：若某阶层怨望或民间请愿成势(见事实清单)，用一个市井/衙署小场景带出(说书人口中的段子、买办的耳语、驿卒捎来的传闻、粮行前的骚动)\n"
      + "  · 势力/阴谋伏笔：暗线自然融入\n"
      + "  · 本回合最戏剧性的一幕必须展开写足\n"
      + "【禁止】\n"
      + "  · 不用emoji\n"
