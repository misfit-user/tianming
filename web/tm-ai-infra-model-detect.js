...
async function probeModelEvidenceAudit(opts) {
  ...
  var _memoryPolicy = typeof window !== 'undefined' && window.TM && window.TM.MemoryAdaptive;
  var _memoryLease = _memoryPolicy ? _memoryPolicy.beginProbe(_tier) : null;
  var startedAt = Date.now();
  var report = {
    tier: _tier,
    profile: 'tm-realistic-evidence-v3',
    model: _aiCfg.model || '',
    responseModel: '',
    checks: [],
    warnings: [],
  ...
  function _addCheck(id, label, ok, detail, extra) {
    extra = extra || {};
    var weight = Number(extra.weight == null ? 10 : extra.weight);
    var row = {
      id: id,
      label: label,
      ok: !!ok,
  ...
  async function _chat(label, messages, maxTokens, timeoutMs, extraBody) {
    var attempt = 0;
    var lastErr = null;
    while (attempt < 2) {
      ...
      try {
        ...
        var resp = await (typeof _tmAIFetch === 'function' ? _tmAIFetch : fetch)(chatUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key },
          body: JSON.stringify(Object.assign({ model:_aiCfg.model || '', messages:messages, temperature:0, max_tokens:maxTokens || 256, stream:false }, extraBody || {})),
  ...
  try {
    ...
    var j1 = _memoryPolicy ? _memoryPolicy.strictJSON(r1.text) : _tmProbeJsonParse(r1.text);
    var ok1 = !!(j1 && j1.probe === 'tm-evidence-v1' && j1.sum === 1213 && j1.reverse === 'gnimnait' && Array.isArray(j1.items) && JSON.stringify(j1.items) === "[2,4,6,8]" && j1.truth === true);
  ...
  } catch(e1) { _addCheck('json_schema', '基础严格 JSON/算术/字段', false, e1.message || e1, { weight:0, state:'unavailable' }); }
  ...
  try {
    ...
    var j2 = _memoryPolicy ? _memoryPolicy.strictJSON(r2.text) : _tmProbeJsonParse(r2.text);
  ...
  } catch(e2) { _addCheck('endturn_schema', '天命回合结构化小样', false, e2.message || e2, { weight:0, state:'unavailable' }); }
  ...
  try {
    ...
    var j3 = _memoryPolicy ? _memoryPolicy.strictJSON(r3.text) : _tmProbeJsonParse(r3.text);
  ...
  } catch(e3) { _addCheck('repair_resilience', '坏 JSON 修复能力', false, e3.message || e3, { weight:0, state:'unavailable' }); }
  ...
  try {
    ...
    var filler = fillerUnit.repeat(Math.max(100, Math.floor(Math.min(24000, Math.max(2000, Number(opts.contextChars) || 10000)) / fillerUnit.length)));
    ...
    var j4 = _memoryPolicy ? _memoryPolicy.strictJSON(r4.text) : _tmProbeJsonParse(r4.text);
  ...
  } catch(e4) { _addCheck('context_recall', '长上下文首尾与新旧信息', false, e4.message || e4, { weight:0, state:'unavailable' }); }
  ...
  try {
    ...
    var j5 = _memoryPolicy ? _memoryPolicy.strictJSON(r5.text) : _tmProbeJsonParse(r5.text);
  ...
  } catch(e5) { _addCheck('narrative_record', '时政记/实录叙事样本', false, e5.message || e5, { weight:0, state:'unavailable' }); }
  ...
  try {
    ...
    var matches = Array.from(new Set(r6.text.match(re) || []));
  ...
  } catch(e6) { _addCheck('output_sustain', '持续输出可控文本', false, e6.message || e6, { weight:0, state:'unavailable' }); }

  if (_memoryPolicy) await _memoryPolicy.runProbes(_chat, _addCheck, opts);
  ...
  if (_memoryLease && !_memoryPolicy.commitEvidence(report, _memoryLease)) return report;
  if (!P.conf._probeHistory) P.conf._probeHistory = {};
