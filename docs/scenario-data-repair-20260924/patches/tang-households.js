...
const SOURCE = require(path.join(DIR, 'sources/xintangshu-dili-37-43.json'));
// 《元和郡县图志》元和户（约 813 年，离开成五年最近）；河朔三镇不上户籍于朝廷，元和志所载不可用
...
// 唐廷与河朔、昭义四镇的行政树
const TREES = ['player', '唐·魏博镇', '唐·成德镇', '唐·卢龙镇', '唐·昭义镇'];
...
function sourceFor(leafName, counties) {
  ...
  return SOURCE.filter((r) => r.nameS === want.name && (!want.circuit || r.circuitS === want.circuit));
}
