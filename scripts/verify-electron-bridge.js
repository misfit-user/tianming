...
const modes = argv.includes('--authoring-continuation') ? ['authoring-continuation'] : argv.includes('--memorial-reading') ? ['memorial-reading'] : argv.includes('--authoring-efficiency') ? ['authoring-efficiency'] : argv.includes('--authoring-recovery') ? ['authoring-recovery'] : argv.includes('--authoring-boundaries') ? ['authoring-boundaries'] : argv.includes('--authoring-stream') ? ['authoring-stream'] : argv.includes('--rail-badges') ? ['rail-badges'] : argv.includes('--character-actions') ? ['character-actions'] : argv.includes('--edict-clarity') ? ['edict-clarity'] : argv.includes('--relief-inspect') || argv.includes('--relief-inspect-small') ? ['relief-inspect'] : argv.includes('--relief-pilot') ? ['relief-pilot'] : argv.includes('--authoring-regions') ? ['authoring-regions'] : argv.includes('--edict-polish') ? ['edict-polish'] : argv.includes('--building-appraisal') ? ['building-appraisal'] : argv.includes('--baseline') ? ['production', 'test-exports'] : ['production', 'test-exports', 'restart'];
