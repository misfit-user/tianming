...
(function(root){'use strict';
  ...
  function makeRenderer(gl,prog){
    ...
    function render(w,ex,mvp,cam,dist,time,detailed,shadows){if(!w)return;build(w,ex,detailed);gl.useProgram(p);gl.enable(gl.DEPTH_TEST);gl.depthMask(true);gl.disable(gl.BLEND);gl.uniformMatrix4fv(locations.uMVP,false,mvp);gl.uniform2f(locations.uCam,cam.x,cam.y);gl.uniform3fv(locations.uSky,[.67,.73,.72]);gl.uniform1f(locations.uFn,dist*1.1);gl.uniform1f(locations.uFf,dist*2.7);gl.uniform1f(locations.uTime,time);gl.uniform1f(locations.uWater,0);if(shadows)shadows.bind(p);for(const c of chunks)if(Math.hypot(c.center.x-cam.x,c.center.y-cam.y)<dist*2.5+5000)drawMesh(c);if(water){gl.uniform1f(locations.uWater,1);drawMesh(water);}}
