...
const dir=path.join(work,'native-ui-software');fs.mkdirSync(dir,{recursive:true});
fs.mkdirSync(path.join(dir,'isolated-user-data'),{recursive:true});fs.mkdirSync(path.join(dir,'isolated-session-data'),{recursive:true});app.disableHardwareAcceleration();
...
win.webContents.on('render-process-gone',(e,details)=>{report.crash=details;report.error='Renderer exited';finish(1);});
