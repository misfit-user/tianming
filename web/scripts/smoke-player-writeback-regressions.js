...
for(const text of ['甲与乙成婚。','本月正式与邻国缔盟。','本月正式颁诏施行新制。','本月未遣使；但今日正式遣使缔盟。'])test('real unlanded event still blocks: '+text,()=>{
 ...
 const r=c.applyAITurnChanges({_strictValidation:true,shizhengji:text,changes:[{path:'custom.score',delta:5}]});assert.equal(r.ok,false);assert.equal(r.rolledBack,true);assert.equal(c.GM.custom.score,10);assert(r.applied.failed.length>0);}
});
for(const key of ['changes','updates'])test('marriage recognizes actual '+key+' effects and not an unrelated field',()=>{
 ...
 c.GM.chars[0].spouse='';c.GM.chars[1].spouse='';const bad=c.applyAITurnChanges({_strictValidation:true,shizhengji:'甲与乙成婚。',char_updates:[{characterId:'a',updates:{loyalty:60}}]});assert(!bad.ok);assert.equal(c.GM.chars[0].loyalty,40);
});
...
for(const [text,shouldApply] of [['本月不但颁诏，还命百官遵行。',false],['上年拟定的新法本月正式颁诏施行。',false],['朝廷不得不正式与邻国缔盟。',false],['本月计划正式颁诏。',true],['本月不但没有颁诏，也未遣使。',true]])test('affirmation and time qualification: '+text,()=>{
 const c=writer();c.GM=baseGM({custom:{score:10}});const result=c.applyAITurnChanges({_strictValidation:true,shizhengji:text,changes:[{path:'custom.score',delta:5}]});assert.equal(result.ok,shouldApply);assert.equal(c.GM.custom.score,shouldApply?15:10);
});
