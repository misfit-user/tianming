...
(function(global){
  ...
  var _dragging = false;
  var _enabled = true;
  var _baseCanvas = null, _baseCtx = null, _baseDirty = true, _baseMap = null, _baseDivisions = null;
  var _baseCount = -1, _baseWidth = 0, _baseHeight = 0, _cameraKey = null;
  ...
  function init(){
    ...
    // Camera has no event: poll only its small signature; retain the static map surface.
    setInterval(function(){
      if (_enabled) render(false);
    }, 100);
  ...
  function render(force){
    if (force !== false) _baseDirty = true;
    if (!_ctx || !_canvas || !_enabled || document.hidden || _canvas.isConnected === false) return;
    var editor = ME.EDITOR, map = editor.map, camera = editor.camera || {}, main = editor.canvas || {};
    var divisions = map.divisions || [], width = map.bitmapWidth || 1280, height = map.bitmapHeight || 800;
    if (_baseMap !== map || _baseDivisions !== divisions || _baseCount !== divisions.length ||
        _baseWidth !== width || _baseHeight !== height || !_baseCanvas ||
        _baseCanvas.width !== _canvas.width || _baseCanvas.height !== _canvas.height) _baseDirty = true;
    var key = [camera.x,camera.y,camera.zoom,main.width,main.height,_canvas.width,_canvas.height].join('|');
    if (!_baseDirty && key === _cameraKey) return;
    if (_baseDirty){
      if (!_baseCanvas){ _baseCanvas = document.createElement('canvas'); _baseCtx = _baseCanvas.getContext('2d'); }
      _baseCanvas.width = _canvas.width; _baseCanvas.height = _canvas.height;
      recalcBounds(); renderBase();
      _baseMap = map; _baseDivisions = divisions; _baseCount = divisions.length;
      _baseWidth = width; _baseHeight = height; _baseDirty = false;
    }
    _ctx.drawImage(_baseCanvas, 0, 0); // Restore the base before translucent viewport ink.
    renderViewport(); _cameraKey = key;
  }
