...
在后续 P00 只读审计中发现：原快测会临时用副 API 的 key/url/model 接管主传输槽，但没有同步 thinking。已补齐 thinking 与协议的临时切换，结束时恢复原值以及原本是否存在这些可选字段；超时后的清凭据与销毁沙盒规则不变。

同一份新回归运行旧 `c229f076` 桥得到 **54 PASS / 4 FAIL**，运行修复桥得到 **58 PASS / 0 FAIL**。覆盖主开副关、主关副开、副默认和主默认四种组合，均验证实际参数构造与退出后的原配置。补充门禁检查还发现启动脚本测试首次提及拆分文件的顺序不符架构契约，已仅重排断言位置，全部断言保留；最终结果见补充证据。

补充后的最终全量为 `ci-uyzAk8`：**948 PASS / 0 FAIL / 0 SKIP / 2 既有 WAIVED，共950项**；完整回合52.9秒、锁恢复8.6秒。架构13、综合Electron23、parity27、发布契约166、构建闸27、含410资产的canonical1101均再次通过。补充源码与日志哈希见 `docs/evidence/seven-ui-thinking-tier-20260912.json`。新方案仅开始只读P00输入审计，未纳入本次七项修复提交。
