...
if(footer<0)throw Error('Storage boundary missing');fs.writeFileSync(path.join(dir,'browser-storage-copy.js'),source.slice(0,footer));
const files={'/test.html':['browser-test.html','text/html; charset=utf-8'],'/browser-storage-copy.js':['browser-storage-copy.js','text/javascript; charset=utf-8']};
...
const report={at:new Date().toISOString(),sourceSHA:crypto.createHash('sha256').update(source).digest('hex'),version,isolatedProfile:profile,outcome,error};
