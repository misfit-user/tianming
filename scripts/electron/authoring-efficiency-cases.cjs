...
module.exports=async function({win,root,check}){
 ...
 await check('real editor stops repeated reads, keeps draft and shows actual request statistics',async()=>{
  await js(`(()=>{TM_SCENARIO_EDITOR_RESET_APP.applyImportedScenario({id:'spin',name:'原',labels:[],fiscalConfig:{treasury:100}},'隔离测试');localStorage.setItem('tm_api',JSON.stringify({url:'https://efficiency.invalid',key:'synthetic-only',model:'controlled'}));window.__eff={calls:0,tools:[]};window.fetch=async function(u,o){if(String(u).indexOf('https://efficiency.invalid')!==0)throw Error('network denied');const b=JSON.parse(o.body),names=(b.tools||[]).map(t=>t.function.name);const response=(name,input)=>new Response(JSON.stringify({choices:[{message:{tool_calls:[{id:'f',type:'function',function:{name,arguments:JSON.stringify(input)}}]},finish_reason:'tool_calls'}],usage:{prompt_tokens:10,completion_tokens:2,total_tokens:12}}));if(names.length===1&&names[0]==='setTitle')return response('setTitle',{title:'隔离核对'});if(names.length===1&&names[0]==='selectMemories')return response('selectMemories',{names:[]});__eff.calls++;__eff.tools.push(names);return response('getField',{path:'name'});};document.getElementById('tm-aa-fab').click();TM_AuthoringAgentUI.permMode('review');const ui=TM_AuthoringAgentUI._ui;ui.els.req.value='修改名称';ui.els.go.click();})()`);
  await until(`!TM_AuthoringAgentUI._ui.running&&__eff.calls>0`);
  const r=await js(`(()=>{const ui=TM_AuthoringAgentUI._ui;TM_AuthoringAgentUI.showUsage();return{calls:__eff.calls,tools:__eff.tools[0].length,toolNames:__eff.tools,expectedTools:TM.AuthoringAgent.AGENT_TOOLS.map(t=>t.name).sort(),status:ui.els.status.textContent,source:TM_SCENARIO_EDITOR_RESET_APP.state.scenario.name,draft:ui.draft.name,metrics:ui._lastRunMeta.metrics,usage:document.querySelector('.tm-aa-request-metrics')?.textContent};})()`);
  // Editing now intentionally offers the complete catalog by default. Assert
  // every actual provider request matches it, not the retired <24-tool cap.
  assert(r.expectedTools.includes('listGaps')&&r.expectedTools.includes('multiEdit'));
  assert.equal(r.toolNames.length,r.calls);r.toolNames.forEach(names=>assert.deepEqual(names.slice().sort(),r.expectedTools));
  assert(r.calls<20);assert.equal(r.tools,r.expectedTools.length);assert.equal(r.source,'原');assert.equal(r.draft,'原');assert.equal(r.metrics.httpRequests,r.calls);assert.equal(r.metrics.inputTokens,r.calls*10);assert.match(r.usage,/服务商已报告/);observations.push({kind:'controlled-spin',...r,status:undefined,usage:undefined});
 });
